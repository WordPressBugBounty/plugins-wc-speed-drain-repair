...
add_action('admin_enqueue_scripts', function ($hook) {
    if ($hook === 'settings_page_repair_woocommerce_speed_settings') {
        wp_enqueue_style(
...
add_action('admin_init', function () {
    if (get_transient('_repair_woocommerce_speed_do_redirect')) {
        ...
        wp_safe_redirect(admin_url('options-general.php?page=repair_woocommerce_speed_settings'));
        exit;
    }
});
...
">Setting Saved</div>
    ...
    <img src="<?php echo plugins_url('assets/images/woo.png', __FILE__); ?>" alt="WooCommerce Icon" style="height: 32px; width: auto;">
    WooCommerce Speed Repair Settings
</h1>
...
<img src="<?php echo plugins_url('assets/images/info.png', __FILE__); ?>" style="max-width: 100%;margin: 0 auto;display: block;" loading="lazy" decoding="async">
...
if ($group_title === 'Mini Cart Assets') {
    ...
    $highlight = '<span style="color: red;font-weight:700">
        Keep these disabled if you are using a mini cart feature on the site.
    </span>';
...
function debug_woocommerce_assets_admin_bar($wp_admin_bar) {
    ...
    $wp_admin_bar->add_node([
        ...
        'href'   => admin_url('options-general.php?page=repair_woocommerce_speed_settings'),
    ]);
}